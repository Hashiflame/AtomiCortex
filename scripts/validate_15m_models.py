...
# None -> use MLStrategyConfig15M().confidence_threshold.
...
#   _MIN_TRADES = {"trend": 1500, "orb": 500}
# Arbitrary fixed floors over-penalised genuinely rare regimes (ORB).
# The gate now checks statistical *precision* of the win-rate estimate.
...
#  iterations, single vs MULTI symbol, 2 model types × 16 WF windows).
...
def is_trade_count_sufficient(
    ...
    Instead of an arbitrary fixed threshold (old 1500/500), checks
    ...
    For 5% CI: n>=384 at WR=0.5, n>=288 at WR=0.75 (rarer regimes like
    ORB that are also more accurate need fewer trades — correct).
...
def validate_model(
    ...
    # NOTE: with --oos-start this is rows after that date (honest OOS
    # iff the model was retrained excluding the period). Without it,
    # the last 20% — the same slice used as the training test set, and
    # IN-SAMPLE for a MULTI model on a single symbol. The authoritative
    # OOS evidence is the walk-forward result computed in step 3 below.
...
def main() -> None:
    ...
    for model_type in ["trend", "orb"]:
        ...
        model_path = path_15m(models_dir, model_type)
        dataset_path = dataset_base / f"dataset_{model_type}.parquet"

        result = validate_model(
            model_type, model_path, dataset_path, symbol, oos_start
        )
